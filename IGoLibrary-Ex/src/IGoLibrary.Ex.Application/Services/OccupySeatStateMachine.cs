...
internal sealed class OccupySeatStateMachine(
    ...
    ICoordinatorRuntime runtime)
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                if (!ReservationTimeHelper.ShouldReReserve(info.ExpirationTime, runtime.Now))
                {
                    var delay = plan.OccupyRefreshMode == OccupyRefreshMode.FixedTenSeconds
                        ? TimeSpan.FromSeconds(10)
                        : TimeSpan.FromSeconds(runtime.NextInt(10, 21));
    ...
    {
        return SessionAuthFailureDetector.IsSessionInvalidException(ex, runtimeState.Session?.Cookie, runtime.Now);
    }
