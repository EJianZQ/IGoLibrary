...
<p align="center">
  IGoLibrary-Ex 是基于 <code>Avalonia</code> 重构的新一代跨平台桌面端实现。可运行在 <code>Windows 10 22H2</code> 及以上与 <code>macOS 15 Sequoia</code> 及以上。已实现 <strong>扫码获取 Cookie</strong>、<strong>座位实时监控并抢座</strong>、<strong>跨场馆全域捡漏</strong>、<strong>明日预约</strong>、<strong>利用退座机制进行占座</strong>、<strong>手机远程控制</strong>、<strong>远程蓝牙签到</strong> 和 <strong>自动事件提醒</strong> 等实用功能
</p>
...
- 🔐 基于微信扫码链接来获取 Cookie 完成登录，支持复制链接自动识别、局域网或 Cloudflare Tunnel 快传，并支持恢复本地保存的会话
...
- 📱 支持通过局域网或 Cloudflare Tunnel 使用手机控制，远程查看 Cookie、预约和任务状态，刷新 Cookie，启动抢座、全域捡漏或占座，以及取消任务或当前预约
...
### 1️⃣ 获取 Cookie
进入 `账户与场馆` 页面后，可以用下面两种方式获取 Cookie。两种方式都需要使用已经在微信绑定好“我去图书馆”学校等信息的微信。

#### 方法一：复制链接到电脑
1. 在 `账户与场馆` 页面使用已经在微信绑定好"我去图书馆"学校等信息的**微信**扫描软件内的二维码
2. 扫码后会进入“我去图书馆”的授权页面，点击页面右上角的 `...`，然后点击 `复制链接`
3. 将复制后的链接通过任意方式(手机QQ/微信等等)发到**电脑上**然后
4. 在电脑上复制该链接，回到软件的 `账户与场馆` 页面，将会自动识别并填写好 Cookie
5. 如果没有自动识别，也可以把链接粘贴到 `授权链接` 输入框，然后点击 `从链接解析`

#### 方法二：快传（局域网或 Cloudflare Tunnel）
1. 进入 `系统设置 → 网络与接口`，在 `手机控制网络方式` 中选择 `本机局域网` 或 `Cloudflare Tunnel`
   - `本机局域网`：手机和电脑需要连接同一个可互访的局域网。部分校园网、访客网络或开启 AP 隔离的网络可能无法使用；如果系统弹出防火墙提示，请允许软件监听当前可信网络
   - `Cloudflare Tunnel`：手机和电脑不需要位于同一局域网，但电脑需要能够连接 Cloudflare；如果不能直连，可以在同一设置页配置系统代理、手动 HTTP 代理或 Clash/Mihomo 兼容模式
2. 回到 `账户与场馆` 页面，点击 `局域网快传` / `公网快传`
3. 弹窗会显示快传网址二维码和手机访问地址
4. 用微信扫描该二维码，打开手机提交页
5. 手机提交页内会显示微信授权二维码。使用已经在微信绑定好"我去图书馆"学校等信息的**微信**长按识别该二维码；如果长按不可用，可以截图后在微信扫一扫中识别
6. 扫码后会进入“我去图书馆”的授权页面，点击页面右上角的 `...`，然后点击 `复制链接`，复制包含 `code` 的授权链接
7. 返回手机提交页
8. 在手机提交页点击 `点击粘贴链接`。如果微信浏览器限制读取剪贴板，就手动粘贴授权链接
9. 点击 `发送到电脑`。提交成功后，电脑端会自动获取 Cookie，刷新授权状态和场馆配置，并关闭快传服务
...
### 4️⃣ 全域捡漏
1. 在 `全域捡漏` 页面点击 `选择扫描场馆` 按钮，勾选希望参与扫描的一个或多个场馆。该功能会扫描场馆内未被加入黑名单的可预约空座，不需要提前指定目标座位
2. 可点击扫描间隔左侧的 `管理黑名单座位`，在已选场馆之间切换并勾选不想预约的座位，点击 `确认保存` 后生效
3. 设置 `扫描间隔`，默认是 `10` 秒。间隔越短扫描越频繁，请根据网络状况和学校接口限制谨慎调整
4. 点击 `开始捡漏` 后把软件挂在后台即可。软件会按轮次扫描已选场馆，发现空座后逐个尝试预约，任意座位预约成功后任务会自动完成
5. 捡漏成功后会写入全域捡漏实时日志，并按通知设置发送本地弹窗、提示音、SMTP 邮件或 Telegram Bot 提醒
...
应用会在本地使用 `SQLite` 保存配置、收藏座位、手机任务启动历史、自定义API接口以及必要的会话信息
