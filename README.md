...
<p align="center">
  IGoLibrary-Ex 是基于 <code>Avalonia</code> 重构的新一代跨平台桌面端实现。可运行在 <code>Windows 10 22H2</code> 及以上与 <code>macOS 15 Sequoia</code> 及以上。已实现 <strong>扫码获取 Cookie</strong>、<strong>座位实时监控并抢座</strong>、<strong>场馆空座追踪</strong>、<strong>明日预约</strong>、<strong>利用退座机制进行占座</strong>、<strong>防漏签到守护</strong> 和 <strong>Cookie 状态检测提醒</strong> 等实用功能
</p>
...
- 👀 支持场馆空座追踪，满座后持续检测，出现空座时提醒
- 📅 支持明日预约，可设置触发时间到点执行
- ♻️ 支持占座流程，在预约即将到期时自动取消并重新预约
- ⏱️ 支持防漏签到守护，可按签到截止时间提醒，错过后按策略仅提醒、自动退座或补约座位
- ⭐ 支持收藏常用座位，并为每个场馆分别持久化座位收藏
- 📊 首页面板展示当前场馆、预约状态、累计成功次数和守护时长
- 🔔 支持 Cookie 失效、抢座成功、占座成功、明日预约成功、空座出现、签到提醒、错过签到、签到补约成功和任务失败提醒，可通过右下角 Toast 弹窗、提示音、SMTP 邮件、Telegram Bot 和 Bark 通知用户
...
4. 如果担心忘记签到，可在同页设置 `签到截止时间`、`提前提醒` 和 `错过签到策略` 后点击 `开始签到守护`。策略支持只提醒、不退座，或到点自动退座并尝试重新预约原座；如果原座失败，也可以改为在当前锁定场馆内随机补约空座

### 6️⃣ 系统提醒
系统提醒目前包含 Cookie 失效提醒、抢座成功提醒、占座成功提醒、明日预约成功提醒、场馆空座提醒、签到提醒、错过签到提醒、签到补约成功提醒和任务失败提醒，可按需开启本地弹窗、提示音、SMTP 邮件、Telegram Bot 和 Bark 通知。

Cookie 状态以真实接口校验结果为准。软件会显示最近一次校验状态与时间；如果能从 Token 中解析出时间，只会作为“Token 推测时间”辅助展示，不作为 Cookie 是否失效的最终依据。
####  邮件提醒配置
...
开启后，Cookie 失效、抢座成功、占座成功、明日预约成功、空座出现、签到提醒和任务失败都会通过 Telegram Bot 发送提醒。Telegram 发送失败时只会写入应用日志，不会阻塞本地弹窗、邮件或任务执行流程
#### Bark 提醒配置
1. 在 `通知设置` 页面切换到 `Bark` 配置页
2. 开启 `Bark 提醒`
3. 填写 `服务器地址` 和 `Device Key`
   - `服务器地址` 默认可使用 `https://api.day.app`
   - 如果使用自建 Bark 服务，请改成自己的服务地址
4. 按需设置提示音和分组
5. 点击 `测试 Bark`，确认能够收到测试消息后即可保存使用

开启后，Cookie 失效、抢座成功、占座成功、明日预约成功、空座出现、签到提醒和任务失败都可以通过 Bark 推送到手机。Bark 发送失败时只会写入应用日志，不会阻塞本地弹窗、邮件、Telegram 或任务执行流程
#### 本地弹窗提醒
当监测到 Cookie 失效、抢座成功、占座成功、明日预约成功、空座出现、签到提醒、错过签到、签到补约成功或任务失败时，会在屏幕右下角弹出 Toast 弹窗提醒。如果打开了提示音，还会有相应的提示音
