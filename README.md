...
- 🔐 基于微信扫码链接来获取 Cookie 完成登录，支持复制链接自动识别和局域网快传，并支持恢复本地保存的会话
...
#### 方法二：局域网快传
1. 确认手机和电脑连接在同一个局域网内(部分网络可能不兼容，请用方法一)。如果系统弹出防火墙提示，请允许软件监听局域网
2. 在 `账户与场馆` 页面点击 `局域网快传`
3. 弹窗会直接显示局域网快传网址二维码和手机访问地址
4. 用微信扫描该二维码，打开手机提交页
