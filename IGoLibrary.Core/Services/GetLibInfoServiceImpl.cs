        ...
        public Library? GetLibInfo(string Cookie, string QueryStatement)
        {
            using (var client = new RestClient("https://wechat.v2.traceint.com/index.php/graphql/"))
            {
                ...
                if (responseContent != null)
                {
                    ...
                    {
                        // 手动构造Library对象，因为已移除带参数的构造函数
                        var library = new Library
                        {
                            IsOpen = libRoot.data.userAuth.reserve.libs[0].is_open,
                            Name = libRoot.data.userAuth.reserve.libs[0].lib_name,
                            LibID = libRoot.data.userAuth.reserve.libs[0].lib_id,
                            Floor = libRoot.data.userAuth.reserve.libs[0].lib_floor,
                            SeatsInfo = new SeatsInfo
                            {
                                TotalSeats = libRoot.data.userAuth.reserve.libs[0].lib_layout.seats_total,
                                BookedSeats = libRoot.data.userAuth.reserve.libs[0].lib_layout.seats_booking,
                                UsedSeats = libRoot.data.userAuth.reserve.libs[0].lib_layout.seats_used
                            },
                            Seats = GetLibSeats(libRoot)
                        };
                        return library;
                    }
                    else
